...
// SerialServiceLevels wraps each service in its own level, preserving list
// order, for sequential execution.
func SerialServiceLevels(services []string) [][]string {
	levels := make([][]string, 0, len(services))
	for _, svc := range services {
		levels = append(levels, []string{svc})
	}
	return levels
}
