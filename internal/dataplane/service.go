...
// MissingServiceError wraps a not-found error with the nodeset and service.
...
// NewServiceCache creates an empty service cache.
...
// Get returns a cached service, fetching it on first use.
...
// DedupeServices dedupes each nodeset's services and builds their leveled
// execution plan from service dependencies.
func DedupeServices(ctx context.Context, helper *helper.Helper,
	...
	for i := range nodesets {
		...
		levels, err := BuildServiceLevels(ctx, helper, serviceCache, services)
		if err != nil {
