...
func AssistantPodSpec(
	...
	if instance.Spec.Goose != nil && instance.Spec.Goose.Recipes != nil {
		// Goose discovers local recipe files through GOOSE_RECIPE_PATH. This
		// must be a pod environment variable so it is also present when users
		// exec goose directly, without going through a shell.
		envVars["GOOSE_RECIPE_PATH"] = env.SetValue("/tmp/recipes")
	}
