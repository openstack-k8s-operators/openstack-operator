...
func checkAnsibleVarsFromChanged(
	...
	// Compute current hashes for AnsibleVarsFrom ConfigMaps/Secrets
	currentConfigMapHashes := make(map[string]string)
	currentSecretHashes := make(map[string]string)

	err := deployment.GetNodeSetAnsibleVarsFromHashes(
		ctx, helper, instance, currentConfigMapHashes, currentSecretHashes)
	if err != nil {
		helper.GetLogger().Error(err, "Unable to compute AnsibleVarsFrom hashes")
		return false, err
	}
