...
// This function:
// 1. Checks if all nodes for this service have been updated
// 2. Checks if all nodesets using the same RabbitMQ cluster for this service are updated
// 3. If yes, manages finalizers: adds to current users, removes from old users
func (r *OpenStackDataPlaneNodeSetReconciler) manageServiceFinalizers(
	...
	// Safety check: Ensure all nodes are accounted for in the tracking ConfigMap
	allNodeNames := r.getAllNodeNamesFromNodeset(instance)
	if len(tracking.UpdatedNodes) != len(allNodeNames) {
		Log.Info("Not all nodes accounted for in service tracking, skipping finalizer management",
			"service", serviceName,
	...
	if !allNodesetsUpdated {
		Log.Info("Not all nodesets using the same RabbitMQ cluster have been updated, skipping finalizer removal",
			"service", serviceName)
		return
	}
