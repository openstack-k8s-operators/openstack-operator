...
func (r *OpenStackClientReconciler) Reconcile(ctx context.Context, req ctrl.Request) (result ctrl.Result, _err error) {
	...
	// Reconcile MCP sidecar resources when enabled. The certificate is owned
	// here because independently created OpenStackClient instances can enable
	// MCP without being managed by an OpenStackControlPlane.
	...
	if instance.Spec.MCP != nil && instance.Spec.MCP.Enabled {
		...
		// MCP TLS defaults to enabled independently of the downstream CA bundle.
		mcpTLSEnabled := true
		if mcpTLSEnabled {
