...
func (r *OpenStackClientReconciler) Reconcile(ctx context.Context, req ctrl.Request) (result ctrl.Result, _err error) {
	...
	// Service account, role, binding. The client containers use a fixed non-root
	// UID and RuntimeDefault seccomp profile, which matches the nonroot-v2 SCC.
	...
	// Reconcile MCP sidecar resources when enabled. The MCP server's TLS
	// certificate is owned by the OpenStackControlPlane, which populates
	// Spec.MCPCertSecretName; this controller only consumes it.
	var mcpTLSSvc *tls.Service
	instance.Status.MCPEndpoint = ""
	if instance.Spec.MCP != nil && instance.Spec.MCP.Enabled {
		...
		// When the control plane has provisioned an MCP TLS cert, mount it in
		// the sidecar. Otherwise the MCP server runs without TLS.
		if instance.Spec.MCPCertSecretName != "" {
			mcpTLSSvc = &tls.Service{
				SecretName: instance.Spec.MCPCertSecretName,
		...
		configVars[instance.Name+"-mcp-config"] = env.SetValue(mcpConfigYAML + mcpCloudsYAML)

	} else {
		// MCP is disabled, clean up any previously created MCP config ConfigMap
		mcpConfigCM := &corev1.ConfigMap{
			ObjectMeta: metav1.ObjectMeta{
				Name:      instance.Name + "-mcp-config",
				Namespace: instance.Namespace,
	...
	if instance.Spec.MCP != nil && instance.Spec.MCP.Enabled {
		...
		instance.Status.MCPEndpoint = fmt.Sprintf(
			"%s://%s.%s.svc:8080/openstack/",
			scheme,
			instance.Name+"-mcp",
			instance.Namespace,
		)
...
const (
	...
	mcpCertSecretNameField     = ".spec.mcpCertSecretName"
)
...
func (r *OpenStackClientReconciler) SetupWithManager(
	...
	// index mcpCertSecretName so MCP cert rotation (secret content change) by
	// the control plane triggers a reconcile and the sidecar pod is refreshed
	if err := mgr.GetFieldIndexer().IndexField(ctx, &clientv1.OpenStackClient{}, mcpCertSecretNameField, func(rawObj client.Object) []string {
		cr := rawObj.(*clientv1.OpenStackClient)
		if cr.Spec.MCPCertSecretName == "" {
			return nil
		}
		return []string{cr.Spec.MCPCertSecretName}
	}); err != nil {
		return err
	}
