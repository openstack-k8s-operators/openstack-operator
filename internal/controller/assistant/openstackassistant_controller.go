...
func (r *OpenStackAssistantReconciler) Reconcile(ctx context.Context, req ctrl.Request) (result ctrl.Result, _err error) {
	...
	// Validate the environment CA bundle used by both Lightspeed Stack and MCP.
	if instance.Spec.CaBundleSecretName != "" {
		...
		configVars[instance.Spec.CaBundleSecretName] = env.SetValue(caBundleHash)
	}
	...
	if instance.Spec.Goose != nil {
		if err := openstackassistant.ValidateMCPServerRefs(instance.Spec.Goose.MCPServers); err != nil {
			instance.Status.Conditions.Set(condition.FalseCondition(
				assistantv1.OpenStackAssistantReadyCondition,
				condition.ErrorReason,
				condition.SeverityWarning,
	...
	// Build PodSpec
	spec := openstackassistant.AssistantPodSpec(instance, configVarsHash, resolvedMCPServers)
	...
	// a recipes ConfigMap changes both the mounted volume and the configuration
	// hash exposed to Goose, neither of which can be updated in place.
