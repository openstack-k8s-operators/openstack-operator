...
const (
	assistantFinalizer        = "assistant.openstack.org/finalizer"
	defaultOperatorsNamespace = "openstack-operators"
	operatorLogsRBACSuffix    = "-operator-logs"
)
...
// +kubebuilder:rbac:groups="",resources=endpoints;events;persistentvolumeclaims;pods/log,verbs=get;list;watch
// +kubebuilder:rbac:groups="",resources=nodes;persistentvolumes,verbs=get;list;watch
// +kubebuilder:rbac:groups=apps,resources=daemonsets;replicasets;statefulsets,verbs=get;list;watch
// +kubebuilder:rbac:groups=assistant.openstack.org,resources=*,verbs=get;list;watch
// +kubebuilder:rbac:groups=backup.openstack.org,resources=*,verbs=get;list;watch
// +kubebuilder:rbac:groups=batch,resources=cronjobs,verbs=get;list;watch
// +kubebuilder:rbac:groups=cyborg.openstack.org,resources=*,verbs=get;list;watch
...
// +kubebuilder:rbac:groups=operator.openstack.org,resources=*,verbs=get;list;watch
// +kubebuilder:rbac:groups=storage.k8s.io,resources=storageclasses,verbs=get;list;watch
// +kubebuilder:rbac:groups=test.openstack.org,resources=*,verbs=get;list;watch
...
func (r *OpenStackAssistantReconciler) Reconcile(ctx context.Context, req ctrl.Request) (result ctrl.Result, _err error) {
	...
	if instance.DeletionTimestamp != nil {
		if controllerutil.ContainsFinalizer(instance, assistantFinalizer) {
			clusterRoleName := clusterRBACName(instance)
			if err := r.deleteClusterRBAC(ctx, clusterRoleName); err != nil {
				return ctrl.Result{}, err
			}
			if err := r.deleteOperatorLogsRBAC(ctx, r.operatorNamespace(), operatorLogsRBACName(instance)); err != nil {
				return ctrl.Result{}, err
			}
	...
	if err := r.reconcileOperatorLogsRBAC(ctx, instance, r.operatorNamespace(), operatorLogsRBACName(instance)); err != nil {
		return ctrl.Result{}, err
	}
		...
		if err != nil {
			if k8s_errors.IsNotFound(err) {
				instance.Status.Conditions.Set(condition.FalseCondition(
					...
					"CA bundle secret "+instance.Spec.CaBundleSecretName+" not found or incomplete"))
	...
	// Validate optional ExtraConfig ConfigMaps. Their content is opaque to the
	// operator; each is mounted for the image entrypoint to consume and its
	// hash tracked so content changes trigger a pod restart.
	for _, cfg := range instance.Spec.ExtraConfig {
		_, cfgHash, err := configmap.GetConfigMapAndHashWithName(ctx, helper, cfg.Name, instance.Namespace)
		if err != nil {
			if k8s_errors.IsNotFound(err) {
				instance.Status.Conditions.Set(condition.FalseCondition(
					assistantv1.OpenStackAssistantReadyCondition,
					condition.RequestedReason,
					condition.SeverityInfo,
					assistantv1.OpenStackAssistantExtraConfigWaitingMessage,
					cfg.Name))
				return ctrl.Result{RequeueAfter: time.Duration(10) * time.Second}, nil
			}
			return ctrl.Result{}, err
		}
		configVars[cfg.Name] = env.SetValue(cfgHash)
	}

	// Resolve MCP servers (auto-discover from OpenStackClientRef or use manual URL)
	resolvedMCPServers := make(map[string]string)
	if err := openstackassistant.ValidateCustomMCPServerURLs(instance.Spec.Env); err != nil {
		instance.Status.Conditions.Set(condition.FalseCondition(
			assistantv1.OpenStackAssistantReadyCondition,
	...
	if err := openstackassistant.ValidateMCPServerRefs(instance.Spec.MCPServers); err != nil {
		instance.Status.Conditions.Set(condition.FalseCondition(
			assistantv1.OpenStackAssistantReadyCondition,
			condition.ErrorReason,
			condition.SeverityWarning,
	...
	for _, mcp := range instance.Spec.MCPServers {
		if mcp.OpenStackClientRef != "" {
			...
			mcpURL, err := osclient.GetMCPEndpoint()
			if err != nil {
				instance.Status.Conditions.Set(condition.FalseCondition(
					assistantv1.OpenStackAssistantReadyCondition,
					condition.RequestedReason,
					condition.SeverityInfo,
					"Waiting for OpenStackClient %s MCP endpoint", mcp.OpenStackClientRef))
				return ctrl.Result{RequeueAfter: 10 * time.Second}, nil
			}
			if err := openstackassistant.ValidateMCPServerURL(mcp.Name, mcpURL); err != nil {
				instance.Status.Conditions.Set(condition.FalseCondition(
					assistantv1.OpenStackAssistantReadyCondition,
					condition.ErrorReason,
					condition.SeverityWarning,
	...
	// Build PodSpec
	spec := openstackassistant.AssistantPodSpecWithCABundle(
		instance,
		configVarsHash,
		resolvedMCPServers,
		effectiveCABundleSecretName,
	...
	// Pods have an immutable spec.  Replace an existing pod before attempting
	// reconciliation when its desired spec has changed.  In particular, adding
	// an ExtraConfig ConfigMap changes both the mounted volume and the
	// configuration hash, neither of which can be updated in place.
	...
	if err := r.Get(ctx, client.ObjectKeyFromObject(assistantPod), assistantPod); err != nil {
		if !k8s_errors.IsNotFound(err) {
			return ctrl.Result{}, fmt.Errorf("failed to get OpenStackAssistant pod %s: %w", assistantPod.Name, err)
		}
	} else if !metav1.IsControlledBy(assistantPod, instance) {
		// A pod with the same name already exists but is not owned by this
		// OpenStackAssistant (e.g. an OpenStackClient pod of the same name).
		// Never delete or adopt it.
		instance.Status.Conditions.Set(condition.FalseCondition(
			assistantv1.OpenStackAssistantReadyCondition,
			condition.ErrorReason,
			condition.SeverityWarning,
			assistantv1.OpenStackAssistantReadyErrorMessage,
			fmt.Sprintf("pod %s already exists and is not owned by this OpenStackAssistant", assistantPod.Name)))
		return ctrl.Result{}, nil
	} else if instance.Status.Hash[podSpecHashName] != podSpecHash {
		if err := r.Delete(ctx, assistantPod); err != nil && !k8s_errors.IsNotFound(err) {
			return ctrl.Result{}, fmt.Errorf("error deleting OpenStackAssistant pod %s for configuration change: %w", assistantPod.Name, err)
		}
		Log.Info("OpenStackAssistant pod deleted due to desired configuration change", "pod", assistantPod.Name)
		return ctrl.Result{Requeue: true}, nil
	}
...
// clusterRBACName returns a bounded, deterministic name for the ClusterRole
// and ClusterRoleBinding owned by an OpenStackAssistant instance. ClusterRole
// and ClusterRoleBinding are cluster scoped, so the namespace is hashed in
// alongside the name to avoid collisions between assistants of the same name
// in different namespaces, while keeping the result well within Kubernetes'
// resource name length limit regardless of how long the namespace or CR name are.
func clusterRBACName(instance *assistantv1.OpenStackAssistant) string {
	hash := sha256.Sum224([]byte(instance.Namespace + "/" + instance.Name))
	return fmt.Sprintf("openstackassistant-%s", hex.EncodeToString(hash[:]))
}

// operatorLogsRBACName returns the name of the Role and RoleBinding that grant
// an assistant access to operator pods and their logs. Include the assistant's
// namespace and name in the hash to avoid collisions.
func operatorLogsRBACName(instance *assistantv1.OpenStackAssistant) string {
	return clusterRBACName(instance) + operatorLogsRBACSuffix
}
...
// reconcileOperatorLogsRBAC grants the assistant ServiceAccount read-only
// access to pods and their logs in the OpenStack operator namespace. A Role
// (rather than a ClusterRole) keeps this permission confined to that namespace.
func (r *OpenStackAssistantReconciler) reconcileOperatorLogsRBAC(ctx context.Context, instance *assistantv1.OpenStackAssistant, namespace, name string) error {
	...
	_, err := controllerutil.CreateOrPatch(ctx, r.Client, role, func() error {
		role.Rules = operatorLogsRBACRules()
		return nil
	})
	if err != nil {
		return fmt.Errorf("error reconciling operator logs Role %s: %w", name, err)
	}
	...
	if err != nil {
		return fmt.Errorf("error reconciling operator logs RoleBinding %s: %w", name, err)
	}

	return nil
}

func (r *OpenStackAssistantReconciler) deleteOperatorLogsRBAC(ctx context.Context, namespace, name string) error {
	roleBinding := &rbacv1.RoleBinding{ObjectMeta: metav1.ObjectMeta{Name: name, Namespace: namespace}}
	if err := r.Delete(ctx, roleBinding); err != nil && !k8s_errors.IsNotFound(err) {
		return fmt.Errorf("error deleting operator logs RoleBinding %s: %w", name, err)
	}

	role := &rbacv1.Role{ObjectMeta: metav1.ObjectMeta{Name: name, Namespace: namespace}}
	if err := r.Delete(ctx, role); err != nil && !k8s_errors.IsNotFound(err) {
		return fmt.Errorf("error deleting operator logs Role %s: %w", name, err)
	}

	return nil
}

// operatorNamespace returns the namespace of the operator manager Pod. The
// value is injected through the Kubernetes downward API. The fallback keeps
// local and envtest runs compatible with the default installation namespace.
func (r *OpenStackAssistantReconciler) operatorNamespace() string {
	if r.OperatorNamespace != "" {
		return r.OperatorNamespace
	}
	return defaultOperatorsNamespace
}

func operatorLogsRBACRules() []rbacv1.PolicyRule {
...
const (
	caBundleSecretField = ".spec.caBundleSecretName"
	extraConfigField    = ".spec.extraConfig"
)
...
func (r *OpenStackAssistantReconciler) SetupWithManager(
	...
	if err := mgr.GetFieldIndexer().IndexField(ctx, &assistantv1.OpenStackAssistant{}, extraConfigField, func(rawObj client.Object) []string {
		cr := rawObj.(*assistantv1.OpenStackAssistant)
		names := make([]string, 0, len(cr.Spec.ExtraConfig))
		for _, cfg := range cr.Spec.ExtraConfig {
			if cfg.Name != "" {
...
func (r *OpenStackAssistantReconciler) findAssistantsForOpenStackClient(ctx context.Context, src client.Object) []reconcile.Request {
	...
	for _, item := range crList.Items {
		for _, mcp := range item.Spec.MCPServers {
