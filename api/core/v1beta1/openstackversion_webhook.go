...
func validateMinorUpdateTargetStageAnnotationProgress(old, updated *OpenStackVersion) error {
	if !minorUpdateInProgress(updated) {
		return nil
	}
	oldStage, oldOK := MinorUpdateTargetStageFromAnnotations(old.Annotations)
	newStage, newOK := MinorUpdateTargetStageFromAnnotations(updated.Annotations)
	if !newOK {
		return nil
	}
	...
	if !oldOK {
		...
		if latest >= 0 && newIdx < latest {
			...
			return apierrors.NewForbidden(
				gr, updated.GetName(), &field.Error{
					Type:     field.ErrorTypeForbidden,
					Field:    annotationField,
					BadValue: newStage,
					Detail: fmt.Sprintf(
						...
						newStage, completedStage, updated.Spec.TargetVersion, *updated.Status.DeployedVersion,
					),
				},
			)
		}
	...
	return apierrors.NewForbidden(
		gr, updated.GetName(), &field.Error{
			...
			Detail: fmt.Sprintf(
				...
				oldStage, newStage, updated.Spec.TargetVersion, *updated.Status.DeployedVersion,
			),
		},
	)
}
