...
var _ = Describe("OpenStackOperator Webhook", func() {
	...
	It("Defaults Auth.ApplicationCredentialSecret for parent-level Auth services (Ironic, Watcher)", func() {
		spec := GetDefaultOpenStackControlPlaneSpec()
	...
	When("global application credentials are enabled", func() {
		...
		It("should fill defaults correctly", func() {
			Eventually(func(g Gomega) {
				...
				g.Expect(*cp.Spec.ApplicationCredential.ExpirationDays).To(Equal(365))
				g.Expect(*cp.Spec.ApplicationCredential.GracePeriodDays).To(Equal(182))
				g.Expect(cp.Spec.ApplicationCredential.Roles).To(ConsistOf("service"))
		...
		It("should configure ApplicationCredential with service-specific overrides and global defaults", func() {
			...
			Expect(*global.ExpirationDays).To(Equal(365))
			Expect(*global.GracePeriodDays).To(Equal(182))
			Expect(global.Roles).To(ConsistOf("service"))
