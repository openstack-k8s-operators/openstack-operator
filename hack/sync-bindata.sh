...
kind: ClusterRole
metadata:
  name: ${OPERATOR_NAME}-metrics-auth-role
rules:
- apiGroups:
...
kind: ClusterRole
metadata:
  name: ${OPERATOR_NAME}-metrics-reader
rules:
- nonResourceURLs:
  - /metrics
  verbs:
...
kind: ClusterRoleBinding
metadata:
  name: ${OPERATOR_NAME}-metrics-auth-rolebinding
roleRef:
  apiGroup: rbac.authorization.k8s.io
  kind: ClusterRole
  name: ${OPERATOR_NAME}-metrics-auth-role
...
# Regenerate the bundle-rbac kustomization resource list from the files just
# synced, so a newly added sub-operator is picked up automatically and folded
# into the meta CSV (kustomize has no glob support; the list must be explicit).
{
cat <<'EOF_CAT'
# NOTE: this file is automatically generated by hack/sync-bindata.sh!
#
# Kustomization for sub-operator ServiceAccounts and RBAC. These resources are
# synced from service-operator bundles and folded into the meta
# openstack-operator CSV by operator-sdk generate bundle (--extra-service-accounts).
#
# IMPORTANT: No namePrefix or namespace transformations are applied here.
# The SA names must stay exactly as-is (e.g., barbican-operator-controller-manager)
# because the runtime sub-operator Deployments reference them by those names.

resources:
EOF_CAT
for SA in $(ls ../$BUNDLE_RBAC_DIR/*-serviceaccount.yaml | sort); do
    OP=$(basename "$SA" -serviceaccount.yaml)
    for SUFFIX in serviceaccount leader-election-role leader-election-rolebinding \
                  manager-role manager-rolebinding metrics-auth-role \
                  metrics-auth-rolebinding metrics-reader; do
        echo "- ${OP}-${SUFFIX}.yaml"
    done
done
} > ../$BUNDLE_RBAC_DIR/kustomization.yaml
