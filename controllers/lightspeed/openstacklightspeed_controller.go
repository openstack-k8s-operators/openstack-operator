...
func (r *OpenStackLightspeedReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// If the user did not specify the RAG image, discover one from OpenStackVersion or fall back to the default value
	// from environment variables.
	if instance.Spec.RAGImage == "" {
		versionList := &corev1beta1.OpenStackVersionList{}
		err = r.Client.List(ctx, versionList, &client.ListOptions{Namespace: instance.Namespace})
		if err != nil {
			return ctrl.Result{}, err
		}

		if len(versionList.Items) == 0 || versionList.Items[0].Status.ContainerImages.OpenstackLightspeedImage == nil {
			Log.Info("No RAG image discovered through OpenStackVersion. Using default image: %s",
				lightspeedv1beta1.OpenStackLightspeedDefaultValues.RAGImageURL)
			instance.Spec.RAGImage = lightspeedv1beta1.OpenStackLightspeedDefaultValues.RAGImageURL
		} else {
			versionInstance := &versionList.Items[0]
			instance.Spec.RAGImage = *versionInstance.Status.ContainerImages.OpenstackLightspeedImage
		}
	}

	OLSOperatorInstalled, err := lightspeed.IsOLSOperatorInstalled(ctx, helper)
	if !OLSOperatorInstalled || err != nil {
		errMsg := fmt.Errorf("installation of OpenShift LightSpeed not detected: %w", err)
