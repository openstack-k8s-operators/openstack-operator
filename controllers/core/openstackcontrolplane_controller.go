...
func (r *OpenStackControlPlaneReconciler) reconcileNormal(ctx context.Context, instance *corev1beta1.OpenStackControlPlane, helper *helper.Helper) (ctrl.Result, error) {
	...
	keystoneAPI := &keystonev1.KeystoneAPI{
		ObjectMeta: metav1.ObjectMeta{
			Name:      instance.Name + "-keystoneapi",
			Namespace: instance.Namespace,
		},
	}

	r.Log.Info("Reconciling KeystoneAPI", "ConfigMap.Namespace", instance.Namespace, "keystoneAPI.Name", instance.Name+"-keystoneapi")
	_, err := controllerutil.CreateOrUpdate(ctx, r.Client, keystoneAPI, func() error {
		keystoneAPI.ObjectMeta = metav1.ObjectMeta{
			Name:      instance.Name + "-keystoneapi",
			Namespace: instance.Namespace,
		}
		keystoneAPI.Spec = instance.Spec.KeystoneTemplate
		if instance.Spec.KeystoneTemplate.Secret != "" {
