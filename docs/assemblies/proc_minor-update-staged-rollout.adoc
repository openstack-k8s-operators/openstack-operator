[id="proc_minor-update-staged-rollout_{context}"]
...
`False` with reason `Gated`. There is no dedicated status field for the annotation: the pause
point is the annotation value, and whether the pause is in effect is that `Gated` condition.
The `OpenStackControlPlane` controller skips reconciling control-plane components for stages
beyond the annotation target until you advance or remove the annotation.
...
| `ovn-controlplane`
| OVN control plane images
| No

| `ovn-dataplane`
| OVN controller data plane images on compute nodes
| *Yes* — create an OVN `OpenStackDataPlaneDeployment`
...
| `controlplane`
| All remaining control-plane services
| No

| _(completion)_
| data plane services on compute nodes
| *Yes* — create an `update-services` `OpenStackDataPlaneDeployment`
|===

[NOTE]
Two stages require you to create an `OpenStackDataPlaneDeployment` manually.
...
See <<required-manual-deployments>>.
...
pauses. The `MinorUpdateOVNControlplane` condition becomes `True`. The next stage,
`MinorUpdateOVNDataplane`, is `False` with reason `Gated` and this message:
...
Confirm the pause annotation on the `OpenStackVersion` CR:

[source,bash]
----
$ oc get openstackversion openstack -n openstack \
  -o jsonpath='{.metadata.annotations.core\.openstack\.org/update-target-stage}{"\n"}'
----

Then inspect stage conditions. Include `.reason` so you can tell a pause (`Gated`)
from a stage that is still running (`Requested`):
...
Completed stages show `True`. The currently blocked next stage shows `False` with
reason `Gated` and a message telling you which stage just finished and what to set next.
Reason `Requested` means that stage is still in progress, not paused.
...
When you are ready to update the remaining data plane services on compute nodes, first
create the data plane services update deployment (see <<required-manual-deployments>>),
...
=== Data plane services update deployment

Required before the final completion step can finish. This deployment updates service
containers and the limited set of packages handled by the `update-services` service on
compute nodes.
...
kind: OpenStackDataPlaneDeployment
metadata:
  name: edpm-deployment-update-services
  namespace: openstack
spec:
  nodeSets:
    - openstack-edpm-ipam
...
[source,bash]
----
$ oc apply -f edpm-deployment-update-services.yaml
----
...
[NOTE]
The webhook rejects setting the annotation to a stage that has already been
completed. If you need to pause, choose a stage at or ahead of the current progress. To
see which stages have completed, check the `MinorUpdate*` conditions on the
`OpenStackVersion` status (`True` is complete; `False` with reason `Gated` is the
paused next stage).
...
. Create the OVN dataplane deployment before or immediately after starting the update.
. Create the data plane services update deployment before the final completion step.
...
Check the pause annotation, then the `MinorUpdate*` conditions.
...
A pause is in effect when the next stage's condition is `False` with reason `Gated`:

[source,bash]
----
$ oc get openstackversion openstack -n openstack -o json | \
  jq '[.status.conditions[] | select(.reason=="Gated" and .status=="False")]'
----

Reason `Requested` means a stage is still running or waiting on a dataplane deployment,
not that the update is paused. Advance or remove the annotation to continue after a
`Gated` pause.
