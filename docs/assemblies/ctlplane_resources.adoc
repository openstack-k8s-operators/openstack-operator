...
| Secret containing the configuration for federated realms This is only needed when multiple realms are federated.
| string
| true

| federationMountPath
| Mount path for federation config files This is only needed when multiple realms are federated. If not specified, "/etc/httpd/conf" is used
| string
| true
|===
