...
By default, OpenStackBaremetalSet automatically creates an `OpenStackProvisionServer` to
serve the bundled OS image for node provisioning. However, you can use a custom provision
server with a different OS image to boot the EDPM nodes. This assumes you have already built
a qcow2 image with all the required packages for EDPM deployment. For instructions on building
custom images, refer to the
https://github.com/openstack-k8s-operators/edpm-image-builder[edpm-image-builder].

The `OpenStackProvisionServer` resource serves OS images (qcow2 format) for baremetal node
provisioning. If you already have a qcow2 image, you can package it in a container image and
use it with the custom provision server.
...
=== Using a Custom Image with OpenStackProvisionServer

After building and pushing your container image, use it with the `OpenStackProvisionServer`
resource:
...
=== Using a Custom Provision Server with OpenStackDataPlaneNodeSet

If you've created a custom `OpenStackProvisionServer` (as shown above), you can reference it
in your `OpenStackDataPlaneNodeSet` resource using the `baremetalSetTemplate` field. The
`OpenStackDataPlaneNodeSet` is managed by the
https://github.com/openstack-k8s-operators/openstack-operator[openstack-operator] and allows
you to specify baremetal host provisioning details.

Simply set the `provisionServerName` field in the `baremetalSetTemplate`:
