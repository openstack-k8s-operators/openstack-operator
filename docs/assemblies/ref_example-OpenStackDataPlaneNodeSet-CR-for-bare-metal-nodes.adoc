...
<1> Configure the bare metal template for bare metal nodes that must be provisioned when creating the resource. Optionally includes `ctlplaneBond` for network interface bonding configuration.
