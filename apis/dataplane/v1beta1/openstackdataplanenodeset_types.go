...
type OpenStackDataPlaneNodeSetSpec struct {
	...
	// +kubebuilder:validation:Optional
	// +kubebuilder:default={bootstrap,configure-network,validate-network,install-os,download-cache,configure-os,ssh-known-hosts,run-os,reboot-os,install-certs,ovn,neutron-metadata,libvirt,nova,telemetry}
	// Services list
