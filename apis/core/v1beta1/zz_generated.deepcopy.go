...
import (
	dataplane_operatorapiv1beta1 "github.com/openstack-k8s-operators/dataplane-operator/api/v1beta1"
...
func (in *OpenStackDataPlaneSpec) DeepCopyInto(out *OpenStackDataPlaneSpec) {
	*out = *in
	if in.Nodes != nil {
		...
		*out = make(map[string]dataplane_operatorapiv1beta1.OpenStackDataPlaneNodeSpec, len(*in))
		for key, val := range *in {
	...
	if in.Roles != nil {
		...
		*out = make(map[string]dataplane_operatorapiv1beta1.OpenStackDataPlaneRoleSpec, len(*in))
		for key, val := range *in {
			(*out)[key] = *val.DeepCopy()
		}
	}
