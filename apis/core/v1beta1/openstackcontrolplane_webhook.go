...
func (r *OpenStackControlPlane) ValidateUpdateServices(old OpenStackControlPlaneSpec, basePath *field.Path) field.ErrorList {
	...
	if r.Spec.Rabbitmq.Enabled {
		...
		for key, rabbitmq := range *r.Spec.Rabbitmq.Templates {

			if oldRabbitmq, ok := oldRabbitmqs[key]; ok {
				errors = append(errors, rabbitmq.ValidateUpdate(oldRabbitmq, basePath.Child("rabbitmq").Child("templates"), r.Namespace)...)
			}
		}
	}
