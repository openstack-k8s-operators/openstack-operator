...
func ReconcileTelemetry(ctx context.Context, instance *corev1beta1.OpenStackControlPlane, helper *helper.Helper) (ctrl.Result, error) {
	...
	op, err := controllerutil.CreateOrPatch(ctx, helper.GetClient(), telemetry, func() error {
		...
		if telemetry.Spec.CeilometerCentral.Secret == "" {
			telemetry.Spec.CeilometerCentral.Secret = instance.Spec.Secret
		}

		if telemetry.Spec.CeilometerCompute.Secret == "" {
			telemetry.Spec.CeilometerCompute.Secret = instance.Spec.Secret
		}
