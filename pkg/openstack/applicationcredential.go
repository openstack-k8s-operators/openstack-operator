...
func ReconcileApplicationCredentials(
	...
	for _, svc := range svcs {
		...
		op, err := controllerutil.CreateOrPatch(ctx, helper.GetClient(), acObj, func() error {
			...
			// build accessRules: use user-supplied if present, otherwise default to
			// “identity /auth/tokens POST” so this AC can only fetch a Keystone token.
			var rules []keystonev1.ACRule
			if svc.ACSection != nil && len(svc.ACSection.AccessRules) > 0 {
				for _, ar := range svc.ACSection.AccessRules {
					if ar.Service == "" {
						continue
					}
					rules = append(rules, keystonev1.ACRule{
						Service: ar.Service,
						Path:    ar.Path,
						Method:  ar.Method,
					})
				...
				rules = []keystonev1.ACRule{{
					Service: "identity",
					Path:    "/auth/tokens",
					Method:  "POST",
				}}
			}
			acObj.Spec.AccessRules = rules
