...
func ReconcileOVN(ctx context.Context, instance *corev1beta1.OpenStackControlPlane, version *corev1beta1.OpenStackVersion, helper *helper.Helper) (ctrl.Result, error) {
	...
	if OVNDBClustersReady && OVNNorthdReady && OVNControllerReady {
		...
		allConditions = append(allConditions, OVNDBClustersConditions...)
		allConditions = append(allConditions, OVNNorthdConditions...)
		allConditions = append(allConditions, OVNControllerConditions...)

		if len(allConditions) > 0 {
			MirrorSubResourceCondition(allConditions, corev1beta1.OpenStackControlPlaneOVNReadyCondition, instance, "OVN")
		} else {
